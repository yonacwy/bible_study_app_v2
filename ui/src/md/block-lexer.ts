...
import { ExtendRegexp } from './extend-regexp.js';
import {
  Align,
  LexerReturns,
  Links,
...
export class BlockLexer<T extends typeof BlockLexer> {
  ...
  protected static rulesBase: RulesBlockBase = null as any;
  /**
   * GFM Block Grammar.
   */
  protected static rulesGfm: RulesBlockGfm = null as any;
  /**
   * GFM + Tables Block Grammar.
   */
  protected static rulesTables: RulesBlockTables = null as any;
  protected rules: RulesBlockBase | RulesBlockGfm | RulesBlockTables = null as any;
  protected options: MarkedOptions;
  protected links: Links = {};
  protected tokens: Token[] = [];
  protected hasRulesGfm: boolean = null as any;
  protected hasRulesTables: boolean = null as any;
  ...
  protected getTokens(src: string, top?: boolean, isBlockQuote?: boolean): LexerReturns {
    ...
    mainLoop: while (nextPart) {
      // newline
      if ((execArr = this.rules.newline.exec(nextPart) as any)) {
      ...
      // code
      if ((execArr = this.rules.code.exec(nextPart) as any)) {
        nextPart = nextPart.substring(execArr[0].length);
      ...
      // fences code (gfm)
      if (this.hasRulesGfm && (execArr = (this.rules as RulesBlockGfm).fences.exec(nextPart) as any)) {
        nextPart = nextPart.substring(execArr[0].length);
      ...
      // heading
      if ((execArr = this.rules.heading.exec(nextPart) as any)) {
        nextPart = nextPart.substring(execArr[0].length);
        this.tokens.push({
      ...
      if (top && this.hasRulesTables && (execArr = (this.rules as RulesBlockTables).nptable.exec(nextPart) as any)) {
        nextPart = nextPart.substring(execArr[0].length);
        ...
        let item_align = item.align ?? [];

        for (let i = 0; i < item_align.length; i++) {
          if (/^ *-+: *$/.test(item_align[i])) {
            item_align[i] = 'right';
          } else if (/^ *:-+: *$/.test(item_align[i])) {
            item_align[i] = 'center';
          } else if (/^ *:-+ *$/.test(item_align[i])) {
            item_align[i] = 'left';
          } else {
            item_align[i] = null as any;
          }
        }
        ...
        for (let i = 0; i < td.length; i++) {
          (item.cells as string[][])[i] = td[i].split(/ *\| */);
        }
      ...
      // lheading
      if ((execArr = this.rules.lheading.exec(nextPart) as any)) {
        nextPart = nextPart.substring(execArr[0].length);
      ...
      // hr
      if ((execArr = this.rules.hr.exec(nextPart) as any)) {
        nextPart = nextPart.substring(execArr[0].length);
      ...
      // blockquote
      if ((execArr = this.rules.blockquote.exec(nextPart) as any)) {
        nextPart = nextPart.substring(execArr[0].length);
      ...
      // list
      if ((execArr = this.rules.list.exec(nextPart) as any)) {
        nextPart = nextPart.substring(execArr[0].length);
        ...
        const str = execArr[0].match(this.rules.item) as RegExpMatchArray;
        const length = (str as RegExpMatchArray).length;
        ...
        for (let i = 0; i < length; i++) {
          let item = (str as RegExpMatchArray)[i];
          ...
          if (this.options.smartLists && i !== length - 1) {
            blockBullet = (this.staticThis.getRulesBase().bullet.exec((str as any)[i + 1]) as any)[0];
      ...
      // html
      if ((execArr = this.rules.html.exec(nextPart) as any)) {
        nextPart = nextPart.substring(execArr[0].length);
      ...
      // def
      if (top && (execArr = this.rules.def.exec(nextPart) as any)) {
        nextPart = nextPart.substring(execArr[0].length);
      ...
      // table (gfm)
      if (top && this.hasRulesTables && (execArr = (this.rules as RulesBlockTables).table.exec(nextPart) as any)) {
        nextPart = nextPart.substring(execArr[0].length);
        ...
        for (let i = 0; i < td.length; i++) {
          (item.cells as any)[i] = td[i].replace(/^ *\| *| *\| *$/g, '').split(/ *\| */);
        }
      ...
      if (this.staticThis.simpleRules.length) {
        ...
        for (let i = 0; i < simpleRules.length; i++) {
          if ((execArr = simpleRules[i].exec(nextPart) as any)) {
            nextPart = nextPart.substring(execArr[0].length);
      ...
      // top-level paragraph
      if (top && (execArr = this.rules.paragraph.exec(nextPart) as any)) {
        nextPart = nextPart.substring(execArr[0].length);
      ...
      if ((execArr = this.rules.text.exec(nextPart) as any)) {
        nextPart = nextPart.substring(execArr[0].length);
