...
import { Links, MarkedOptions, SimpleRenderer, Token, TokenType } from './interfaces.js';
...
export class Parser {
  ...
  protected inlineLexer: InlineLexer = null as any;
  ...
  debug(links: Links, tokens: Token[]) {
    ...
    while (this.next()) {
      const outToken: string = this.tok() ?? null as any;
  ...
  protected next() {
    return (this.token = this.tokens.pop() ?? null as any);
  }
  ...
  protected tok() {
    switch (this.token.type) {
      ...
      case TokenType.heading: {
        return this.renderer.heading(this.inlineLexer.output(this.token.text ?? ""), this.token.depth ?? 0, this.token.text ?? "");
      }
      ...
      case TokenType.table: {
        ...
        // header
        cell = '';
        for (let i = 0; i < (this.token.header as string[]).length; i++) {
          const flags = { header: true, align: (this.token.align as any)[i] };
          const out = this.inlineLexer.output((this.token.header as any)[i]);
        ...
        for (const row of this.token.cells as any) {
          cell = '';

          for (let j = 0; j < row.length; j++) {
            cell += this.renderer.tablecell(this.inlineLexer.output(row[j]), {
              header: false,
              align: (this.token.align as any)[j]
            });
          }
